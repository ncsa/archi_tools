...
def plateau_diff(args):
    ...
    Uniques = StanzaFactory(args,
                            """/*First, we need to get ALL the elements present in EITHER view*/
                                ...
                                LEFT JOIN VIEW_OBJECTS vo on vo.view_id = v.id AND v.name='%s'
                                LEFT JOIN VIEW_OBJECTS vo1 on vo1.view_id = v.id AND v.name='%r'
                                ...
                                FROM TWOVIEWS t
                                LEFT JOIN TWOVIEWS t1 on t1.Object_id = t.Object_id AND t1.name='%s'
                                WHERE t.Name='%r' AND t1.Object_id IS NULL)
